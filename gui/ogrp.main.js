window.addEventListener("load", function () {
	...
	window.addEventListener("message", function (evt) {
		...
		} else if (data.act === "change_bg") {
			changeBackground(data.url);
		} else if (data.act === "change_font") {
	...
	changeBackground = function (url) {
		if (url !== "default" && url !== "RESET") {
			$(".ogrp_mainMenu").css("background", `url(${url})`);
		} else if (url === "RESET") {
			$(".ogrp_mainMenu").removeAttr("style");
		}
	};

	changeFont = function (fontName) {
		$(document.body).css("font-family", fontName);
	};
...
function ogrpDebugBar() {
	$("body").append($("<div>", { id: "ogrpErrDiv", class: "ogrp_console" }));
}

function ogrpMainError() {
	window.onerror = function (errorMsg, url, lineNumber, column, errorObj) {
		$("#ogrpErrDiv").append("<br />Error: " + errorMsg + " Script: " + url + " Line: " + lineNumber + " Column: " + column + " StackTrace: " + errorObj);
	};
}
